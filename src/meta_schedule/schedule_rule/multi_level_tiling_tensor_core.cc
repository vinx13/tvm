...
TensorCoreState::TensorCoreState(tir::Schedule sch, tir::BlockRV block_rv,
...
std::vector<State> MultiLevelTilingTensorCoreNode::AddWriteReuseTensorCore(
  ...
  sch->Annotate(sch->Blockize(tiled_loop), tir::attr::meta_schedule_auto_tensorize,
                intrin_group.store_intrin);
  return {state};
}
...
std::vector<State> MultiLevelTilingTensorCoreNode::AddReadReuseTensorCore(
  ...
  auto f_tensorize_load = [&](int read_index, String scope, String intrin_name) {
    auto cache_block = sch->CacheRead(state->block_rv, read_index, scope);
    state->sch->ComputeAt(cache_block, r_tiles.back(), true);

    Optional<tir::LoopRV> loop = tir::TileWithTensorIntrin(sch, cache_block, intrin_name);
    ICHECK(loop.defined());
    sch->Annotate(sch->Blockize(loop.value()), tir::attr::meta_schedule_auto_tensorize,
                  intrin_name);
...
Optional<LoopRV> MultiLevelTilingTensorCoreNode::TransformWithTensorIntrin(
    TensorCoreState& state, const String& intrin_name) const {
  ...
  for (int i = 0; i < mapping_info_->lhs_iters.size(); ++i) {
  ...
  for (int i = offset; i < index_map->final_indices.size(); ++i) {
...
inline std::vector<State> MultiLevelTilingTensorCoreNode::TransformForTensorization(
  ...
  Optional<LoopRV> transformed_loop_rv =
      TransformWithTensorIntrin(state, intrin_group.compute_intrin);
