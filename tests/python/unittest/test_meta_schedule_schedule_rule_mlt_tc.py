...
def test_conv():
    ...
    class Module:
        @T.prim_func
        def main(p0: T.Buffer[(1, 224, 224, 3), "float16"], p1: T.Buffer[(768, 16, 16, 3), "float16"], p2: T.Buffer[(1, 1, 1, 768), "float16"], T_add: T.Buffer[(1, 14, 14, 768), "float16"]) -> None:
            # function attr dict
            T.func_attr({"global_symbol": "main", "tir.noalias": True})
            # body
            # with T.block("root")
            pad_temp = T.alloc_buffer([1, 224, 224, 3], dtype="float16")
            conv2d_nhwc = T.alloc_buffer([1, 14, 14, 768], dtype="float16")
            ...
            for i0, i1, i2, i3, i4, i5, i6 in T.grid(1, 14, 14, 768, 16, 16, 3):
                with T.block("conv2d_nhwc"):
                    nn, yy, xx, ff, ry, rx, rc = T.axis.remap("SSSSRRR", [i0, i1, i2, i3, i4, i5, i6])
                    T.reads(pad_temp[nn, yy * 16 + ry, xx * 16 + rx, rc], p1[ff, ry, rx, rc])
                    T.writes(conv2d_nhwc[nn, yy, xx, ff])
                    with T.init():
                        conv2d_nhwc[nn, yy, xx, ff] = T.float16(0)
                    conv2d_nhwc[nn, yy, xx, ff] = conv2d_nhwc[nn, yy, xx, ff] + pad_temp[nn, yy * 16 + ry, xx * 16 + rx, rc] * p1[ff, ry, rx, rc]
            for i0, i1, i2, i3 in T.grid(1, 14, 14, 768):
                with T.block("T_add"):
                    ax0, ax1, ax2, ax3 = T.axis.remap("SSSS", [i0, i1, i2, i3])
                    T.reads(conv2d_nhwc[ax0, ax1, ax2, ax3], p2[ax0, 0, 0, ax3])
    ...
    actual = ms.TuneContext(
        ...
        target=tvm.target.Target("cuda"),
        space_generator=ms.space_generator.PostOrderApply(),
        sch_rules=[multi_level_tiling_tensor_core(write_reuse_scope="shared", in_dtype="float16", out_dtype="float16")]
        + get_rules("cuda", ms.schedule_rule.AutoInline),
    ).generate_design_space()
