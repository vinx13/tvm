...
def te_workload():
    X = te.placeholder(shape=(N, PADDED_H, PADDED_W, I), dtype="float16", name='data')
    Weight = te.placeholder(shape=(O, KW, KW, I), dtype="float16", name='weight')
    im2col_data = te.compute(
        (N * H * W, KH * KW * I),
        lambda i, j: X[i // (H * W), (i % (H * W)) // W + j // (KW * I), (i % (H * W)) % W + (j % (KW * I)) // I, j % I],
        name='im2col_data',
    )
...
a_np = np.random.rand(N, H + KH // 2 * 2, W + KW // 2 * 2, I)
b_np = np.random.rand(O, KW, KW, I)
b_np_transpose = b_np.transpose(1, 2, 3, 0)  # OHWI -> HWIO
c_np = tvm.topi.testing.conv2d_nhwc_python(a_np, b_np_transpose, 1, 0).astype('float32')
