...
def schedule_direct_hwcn_cuda(cfg, s, conv):
    """schedule optimized for batch size = 1"""

    ##### space definition begin #####
    y, x, f, n = s[conv].op.axis
    ...
    if cfg.is_fallback:
        ref_log = autotvm.tophub.load_reference_log(
            target.target_name, target.model, 'conv2d', 'direct_hwcn')
        cfg.fallback_with_reference_log(ref_log)
    ##### space definition end #####
    ...
    s[output].reorder(by, bx, bf, vy, vx, vf, ty, tx, tf, yi, xi, fi)
    s[OL].compute_at(s[output], tf)

    # tile reduction axes
    y, x, f, n= s[OL].op.axis
    ry, rx, rc = s[OL].op.reduce_axis
    rco, rci = cfg['tile_rc'].apply(s, OL, rc)
    ryo, ryi = cfg['tile_rx'].apply(s, OL, ry)
    rxo, rxi = cfg['tile_ry'].apply(s, OL, rx)
    s[OL].reorder(ryo, rxo, rco, ryi, rxi, rci, y, x, f, n)
    ...
    # cooperative fetching
    for load in [AA, WW]:
        y, x, f, n = s[load].op.axis
        fused = s[load].fuse(y, x, f, n)
        tz, fused = s[load].split(fused, nparts=cfg["tile_y"].size[2])
        ty, fused = s[load].split(fused, nparts=cfg["tile_x"].size[2])
    ...
    OH, OW, CO, N = get_const_tuple(output.shape)
    KH, KW, _, CI = get_const_tuple(kernel.shape)
    cfg.add_flop(2 * N * OH * OW * CO * CI * KH * KW)
