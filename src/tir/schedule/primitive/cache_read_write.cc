...
Block MakeReIndexStage(const Block& block, CacheStageInfo* info,
                       ...
                       BufferIndexType buffer_index_type, bool keep_unused_block_iters) {
  ...
  for (int i = 0, n = block->iter_vars.size(); i < n; ++i) {
    ...
    if (used || keep_unused_block_iters) {
...
StmtSRef ReIndex(ScheduleState self, const StmtSRef& block_sref, int buffer_index,
  ...
  Block reindex_stage =
      MakeReIndexStage(block, &info, covered, original_indices, buffer_index, buffer_index_type, /*keep_unused_block_iters=*/false);
