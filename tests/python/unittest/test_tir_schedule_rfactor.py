...
def transformed_square_sum_square_root(a: T.handle, d: T.handle) -> None:
    ...
    for i0, i1_i2_fused_outer, i1_i2_fused_inner in T.grid(16, 32768, 2):
        with T.block("C"):
            b = T.axis.S(16, i0)
            i = T.axis.R(256, T.floordiv(i1_i2_fused_outer * 2 + i1_i2_fused_inner, 256))
            j = T.axis.R(256, T.floormod(i1_i2_fused_outer * 2 + i1_i2_fused_inner, 256))
            with T.init():
                C[b] = 0.0
            C[b] = C[b] + (A[b, i, j] * A[b, i, j])
...
def rowsum_zero_dim_rfactor(A: T.Buffer[(128,), "float32"], B: T.Buffer[(), "float32"]) -> None:
    ...
    for k0 in T.serial(128):
        with T.block("B_rf"):
            vk0 = T.axis.spatial(128, k0)
            with T.init():
                B_rf[vk0] = T.float32(0)
            B_rf[vk0] = B_rf[vk0] + A[vk0]
    for k0 in T.serial(128):
        with T.block("B"):
