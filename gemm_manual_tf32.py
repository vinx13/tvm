...
def schedule(sch):
    ...
    sch.annotate(block_or_loop=b20, ann_key="meta_schedule.auto_tensorize", ann_val="wmma_sync_16x16x8_f32f32f32_fast_tf32")
    sch.annotate(block_or_loop=b20, ann_key="meta_schedule.auto_tensorize_init", ann_val="wmma_fill_16x16x8_f32")
    ...
    v44, v45, v46 = sch.sample_perfect_tile(loop=l23, n=3, max_innermost_factor=4, decision=[64, 2, 1])
    ...
    sch.annotate(block_or_loop=b72, ann_key="meta_schedule.auto_tensorize", ann_val="wmma_store_16x16x8_f32_shared")
    ...
    b91 = sch.cache_read(block=b20, read_buffer_index=0, storage_scope="wmma.matrix_a.tf32")
    sch.compute_at(block=b91, loop=l48, preserve_unit_loops=True, index=-1)
    l92, l93, l94, l95, l96, l97, l98 = sch.get_loops(block=b91)
    l99, l100 = sch.split(loop=l98, factors=[None, 8], preserve_unit_iters=True)
    ...
    sch.annotate(block_or_loop=b112, ann_key="meta_schedule.auto_tensorize", ann_val="wmma_load_16x16x8_f32_fast_tf32_a")
    b113 = sch.cache_read(block=b20, read_buffer_index=1, storage_scope="wmma.matrix_b.tf32")
    ...
    l123, l124 = sch.split(loop=l119, factors=[None, 8], preserve_unit_iters=True)
    ...
    sch.annotate(block_or_loop=b134, ann_key="meta_schedule.auto_tensorize", ann_val="wmma_load_16x16x8_f32_fast_tf32_b")
    ...
    sch.annotate(block_or_loop=b233, ann_key="meta_schedule.auto_tensorize", ann_val="wmma_fill_16x16x8_f32")
    ...
    sch.tensorize(block_or_loop=b234, tensor_intrin="wmma_fill_16x16x8_f32")
    b235 = sch.get_block(name="A_reindex_shared_wmma.matrix_a.tf32_o", func_name="main")
    sch.unannotate(block_or_loop=b235, ann_key="meta_schedule.auto_tensorize")
    sch.tensorize(block_or_loop=b235, tensor_intrin="wmma_load_16x16x8_f32_fast_tf32_a")
    b236 = sch.get_block(name="B_reindex_shared_wmma.matrix_b.tf32_o", func_name="main")
    sch.unannotate(block_or_loop=b236, ann_key="meta_schedule.auto_tensorize")
    sch.tensorize(block_or_loop=b236, tensor_intrin="wmma_load_16x16x8_f32_fast_tf32_b")
    b237 = sch.get_block(name="C_o_update", func_name="main")
    sch.unannotate(block_or_loop=b237, ann_key="meta_schedule.auto_tensorize")
    sch.tensorize(block_or_loop=b237, tensor_intrin="wmma_sync_16x16x8_f32f32f32_fast_tf32")
    ...
    sch.tensorize(block_or_loop=b238, tensor_intrin="wmma_store_16x16x8_f32_shared")
...
with target:
    ...
    c_np = a_np @ b_np
    a = tvm.nd.array(a_np, tvm.gpu(0))
    b = tvm.nd.array(b_np, tvm.gpu(0))
    c = tvm.nd.array(np.zeros((M, N), dtype="float32"), tvm.gpu(0))
    func(a, b, c)
    tvm.testing.assert_allclose(c.numpy(), c_np, rtol=1e-3, atol=1e-3)

    time_evaluator = func.time_evaluator(func.entry_name, tvm.gpu(0), number=100)
    mean_ms = time_evaluator(a, b, c).mean * 1000
    GFLOPS = 2 * M * N * K / mean_ms / 1e6
