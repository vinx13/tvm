...
extern "C" __global__ void __launch_bounds__(256) main_kernel0(half* __restrict__ A, half* __restrict__ B, float* __restrict__ C) {
  ...
  __syncthreads();
  // note threadIdx.y = fuse(j, i) instead of fuse(i, j)
  ...
  // this epilogue is exactly same as cutlass
  //
  // pseudo code:
  // each warp has 2x4 fragments
  // loop 2 times:
  //  each warp copy 16x64 tile (4 fragments) to shared memory with shape 64 x (128 + 8)
  //  the thread map is 
  //    warp0, warp4
  //    warp1, warp5
  //    warp2, warp6
