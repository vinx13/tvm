...
/*! \brief Attributes used in convolution backward operators */
struct Conv2DBackwardDataAttrs : public tvm::AttrsNode<Conv2DBackwardDataAttrs> {
  ...
  TVM_DECLARE_ATTRS(Conv2DBackwardDataAttrs, "relay.attrs.Conv2DBackwardDataAttrs") {
    TVM_ATTR_FIELD(data_shape)
        .describe("Data shape of the convolution.");
    TVM_ATTR_FIELD(strides).set_default(Array<IndexExpr>({1, 1}))
        .describe("Specifies the strides of the convolution.");
    TVM_ATTR_FIELD(padding).set_default(Array<IndexExpr>({0, 0}))
        .describe("If padding is non-zero, then the input is implicitly zero-padded"
...
/*! \brief Attributes used in convolution operators */
struct Conv2DBackwardWeightAttrs : public tvm::AttrsNode<Conv2DBackwardWeightAttrs> {
  ...
  TVM_DECLARE_ATTRS(Conv2DBackwardWeightAttrs, "relay.attrs.Conv2DBackwardWeightAttrs") {
    TVM_ATTR_FIELD(weight_shape)
        .describe("Weight shape of the convolution.");
    TVM_ATTR_FIELD(strides).set_default(Array<IndexExpr>({1, 1}))
        .describe("Specifies the strides of the convolution.");
    TVM_ATTR_FIELD(padding).set_default(Array<IndexExpr>({0, 0}))
        .describe("If padding is non-zero, then the input is implicitly zero-padded"
