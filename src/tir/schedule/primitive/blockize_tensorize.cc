...
StmtSRef Blockize(ScheduleState self, const StmtSRef& loop_sref) {
  ...
  BlockRealize inner_block_realize = CheckGetSingleChildBlockRealizeOnSRefTree(self, loop_sref);
  StmtSRef inner_block_sref = self->stmt2ref.at(inner_block_realize.get());
  const Block& block = innter_block_realize->block;
  ...
  auto* inner_block = block.CopyOnWrite();
  inner_block->iter_vars = inner_block_vars;
  inner_block->init = NullOpt;
  auto* inner_br = block_realize.CopyOnWrite();
  inner_br->iter_values = inner_bindings;
  inner_br->predicate = division.back()[1]->extent;
  inner_br->block = inner_block;
