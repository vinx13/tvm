...
@T.prim_func
def matmul_fp16_packed(var_A: T.handle, var_B: T.handle, var_C: T.handle) -> None:
    n = T.var("int32")
    A = T.match_buffer(
    ...
    C = T.match_buffer(var_C, [n, n, 16, 16], elem_offset=0, align=128, offset_factor=1)
    # body
    with T.block([], "root"):
        T.reads([])
        T.writes([])
        for i0, i1, i2, i3, i4, i5 in T.grid(n, n, n, 16, 16, 16):
            with T.block([n, n, T.reduce_axis(0, n), 16, 16, T.reduce_axis(0, 16)], "C") as [
                io,
                jo,
                ko,
                ii,
                ...
                T.bind(ji, i4)
                T.bind(ki, i5)
                T.reads([C[io, jo, ii, ji], A[io, ko, ii, ki], B[ko, jo, ki, ji]])
                T.writes([C[io, jo, ii, ji]])
                with T.init():
                    C[io, jo, ii, ji] = T.float32(0)
                C[io, jo, ii, ji] = C[io, jo, ii, ji] + (
                    T.cast(A[io, ko, ii, ki], "float32") * T.cast(B[ko, jo, ki, ji], "float32")
                )
