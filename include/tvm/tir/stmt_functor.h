 ...
 *
 * Usually we enforce the consistency of data types when constructing the IR nodes. However, there
 * are some cases where the data types of the IR nodes are not consistent with each other when they
 * are mutated by some passes. This class can be used as base class of such passes to ensure the
