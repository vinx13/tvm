...
def te_workload():
    ...
    bias = te.placeholder(shape=(O,), dtype="float32", name="bias")
    ...
    bias_add = te.compute((N, H, W, O), lambda n, h, w, o: conv[n, h, w, o] + bias[o], name='bias_add')
    relu = te.compute((N, H, W, O), lambda n, h, w, o: tir.max(bias_add[n, h, w, o], 0.), name='relu')
    s = te.create_schedule(relu.op)
    return te.create_prim_func([X, Weight, bias, relu]), [X, Weight, conv, bias_add, relu]
...
s.annotate(k0, 'software_pipeline_order', [0,1,3,4,5,2,6])
...
relu = s.get_block('relu')
s.reverse_compute_inline(relu)
bias_add = s.get_block('bias_add')
s.reverse_compute_inline(bias_add)
...
bias_tvm = tvm.nd.array(bias_np.astype('float32'), device=dev)
